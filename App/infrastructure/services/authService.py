...
class AuthService:
    ...
    async def get_auth_group(self, user_id: int) -> ORMAuthgroup:
        result = self.connection.execute(select(ORMAuthgroup).filter(ORMAuthgroup.id == user_id, ORMAuthgroup.deactivate==False))
    ...
    async def delete_auth_group(self, auth_id:int):
        result = await self.get_auth_group(auth_id)
        if result:
            result.deactivate = True
            self.connection.commit()
            self.connection.refresh(result)
            return  result
        return None
